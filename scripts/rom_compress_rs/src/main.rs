...
pub async fn main() -> anyhow::Result<()> {
    ...
    let mut gpu = GpuRuntime::new(&machines, &stimuli).await?;
    gpu.run().await?;
...
impl GpuRuntime<'_> {
    pub async fn new<'a>(machines: &'a [Machine], stimuli: &'a [[u8; size_of::<u32>() * structs::U32_MAX_PER_STAGE]]) -> anyhow::Result<GpuRuntime<'a>> {
        ...
        device_descriptor.required_limits.max_immediate_size = 8;
        ...
        let pipeline_bindgroup_layout0_entries : [wgpu::BindGroupLayoutEntry; 8]= {
            let tmp : Vec<wgpu::BindGroupLayoutEntry> = (0..8u32).into_iter().map(|i| {
